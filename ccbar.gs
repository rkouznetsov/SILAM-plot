*
*  Script to plot a categorcal colorbar
* Made by Roux from cbar
*
...
*  sub-releases.
*
function CatColorbar (args)
*
*  Check shading information
*  args  -- a string of words that are interpreted as categories
*           number of words should be equal to number of colors
*           (ie nuber_of_clevs+1)
*
  'query shades'
  shdinfo = result
...
*subwrd(args,num)
    ...
    if (vert) 
      'draw string '%(xr+0.05)%' 'yc' 'lab
    else
      'draw string 'xc' '%(yb-0.05)%' 'lab
    endif
    num = num + 1
    if (vert); yb = yt;
    else; xl = xr; endif;
